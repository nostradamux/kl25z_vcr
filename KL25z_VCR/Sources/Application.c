...
extern volatile uint32_t DataTachometerCapture;
	...
	extern LDD_TDeviceData *TachometerSimulation;
#endif
...
{
	TachometerSimulation=Tacometer_Simulator_Init(NULL);
}
void Set_Tacometer_Pulses()
{
	...
	static uint16_t periodTacometerOld=0;
		...
		if (movement.tacho!=periodTacometerOld)
		{
			...
			Tacometer_Simulator_SetPeriodMS(TachometerSimulation, movement.tacho);
			Tacometer_Simulator_SetRatio16(TachometerSimulation, 0xFFFF/2);
		}
}
#else
